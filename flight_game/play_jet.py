...
play_jet.py
------------
Lesson 2: the Fighter Jet. Run with `python play_jet.py` from the project
root (or `python -m flight_game.play_jet`).

Same shape as play_plane.py — the physics (vehicles.py) and renderer
(render.py) are shared across every vehicle. Only the vehicle config and
the duck instructor's lesson content are lesson-specific.
"""
...
from .vehicles import FIGHTER_JET_CONFIG, make_initial_state, step_physics, Controls
from .duck_instructor_jet import DuckInstructorJet
from . import render

CONTROLS_HINT = "Up/Down: Nose   W/S: Throttle   Shift: Afterburner   R: Restart   Esc: Quit"
...
def main():
    ...
    pygame.display.set_caption("Duck Aerospace Academy — Lesson 2: Fighter Jet")
    clock = pygame.time.Clock()
    font = pygame.font.SysFont("consolas", 18)
    font_small = pygame.font.SysFont("consolas", 15)

    config = FIGHTER_JET_CONFIG
    state = make_initial_state(config)
    instructor = DuckInstructorJet()
    ...
    while running:
        ...
        controls = Controls(
            ...
            afterburner=bool(keys[pygame.K_LSHIFT] or keys[pygame.K_RSHIFT]),
        )
